...
function initGUI() {
	...
	gui.add(settings, "input_symmetry", symmetry_types)
		.onChange((type) => {
	...
	smokeFolder
		.add(settings, "light_height", 0.5, 1, 0.001)
		.name("Light Elevation");
	smokeFolder
		.add(settings, "light_intensity", 0, 1, 0.001)
		.name("Light Intensity");
	smokeFolder
		.add(settings, "light_falloff", 0.5, 10, 0.001)
		.name("Light Falloff");
	smokeFolder.add(settings, "enable_shadows").name("Enable Shadows");
	smokeFolder
		.add(settings, "shadow_intensity", 0, 50, 0.001)
